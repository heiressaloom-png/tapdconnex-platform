...
function bind(){let a=id=>document.getElementById(id);if(S.screen==='ready'){a('capBtn').onclick=()=>{if(!(S.ev&&S.ev.name&&S.ev.location)){eventBox();return;}start()};if(a('tplBtn'))a('tplBtn').onclick=()=>{location.href='templates.html'};if(a('topTemplates'))a('topTemplates').onclick=()=>{location.href='templates.html'};(a('evBtn')||{}).onclick=eventBox;(a('evBtn2')||{}).onclick=eventBox}if(S.screen==='recording'){document.getElementById('actionBar').style.display='none';bindOneRecordingButton();let q=document.getElementById('recQuestions');if(q)q.onclick=e=>{let li=e.target.closest('li');if(li)li.classList.toggle('asked')}}if(S.screen==='review'){document.getElementById('actionBar').style.display='block';document.getElementById('actionInner').innerHTML='<button class="btn btn-secondary" id="discard">Discard</button><button class="btn btn-primary" id="save">Save to Relationship Hub</button>';a('save').onclick=save;a('discard').onclick=()=>{S.p={};S.ai=null;S.aiStatus='idle';S.screen='ready';render()}}if(S.screen==='saved')a('again').onclick=()=>{S.p={};S.ai=null;S.aiStatus='idle';S.transcript='';S.blob=null;S.screen='ready';render()}}
async function start(){if(!(S.ev&&S.ev.name&&S.ev.location)){eventBox();return;}S.chunks=[];if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia||typeof MediaRecorder==='undefined'){S.screen='review';render();return}try{S.stream=await navigator.mediaDevices.getUserMedia({audio:true});S.rec=new MediaRecorder(S.stream);S.rec.ondataavailable=e=>{if(e.data&&e.data.size)S.chunks.push(e.data)};S.rec.onstop=finish;S.rec.start();S.t0=Date.now();S.paused=false;S.pausedMs=0;S.pauseStart=0;S.screen='recording';render();S.timer=setInterval(()=>{let el=document.getElementById('time');if(el){let now=S.paused?S.pauseStart:Date.now();let s=Math.max(0,Math.floor((now-S.t0-S.pausedMs)/1000));el.textContent=Math.floor(s/60)+':'+String(s%60).padStart(2,'0')}},250)}catch(e){S.screen='review';render()}}
...
/* ============================================================
   ...
   ============================================================ */
function finish(){try{if(S.chunks.length)S.blob=new Blob(S.chunks,{type:'audio/webm'})}catch(e){}if(S.stream)S.stream.getTracks().forEach(t=>t.stop());runPipeline()}
...
async function runPipeline(){
  ...
  S.p={name:'',company:'',role:'',summary:S.transcript.slice(0,600),signal:'',momentum:'',nextStep:''};
...
async function postJSON(url,body){const res=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});if(!res.ok)return null;const j=await res.json();if(j&&j.status==='placeholder')return null;return j}
...
function aiToReviewForm(ai){
  ...
  return {
    ...
    signal:'',
    momentum:momentum,
    nextStep:(ai.draft&&ai.draft.nextStep)||''
  };
}
...
function save(){try{pullForm();let t=tpl(),e=S.ev,p=S.p,ai=S.ai,evText=e&&e.name?[e.name,e.session,e.location].filter(Boolean).join(' · '):'No event set';
  ...
  let priority=aiPriority(ai,pri(p.momentum,p.signal));
  ...
  let c={id:id(),name:p.name||null,company:p.company||null,role:p.role||null,
    ...
    transcriptConfidence:S.transcriptConfidence||null,
