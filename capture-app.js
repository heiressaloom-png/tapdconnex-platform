...
function render(){document.getElementById('actionBar').style.display='none';document.getElementById('view').innerHTML=S.screen==='ready'?ready():S.screen==='recording'?recording():S.screen==='processing'?processing():S.screen==='gutfeel'?gutfeel():S.screen==='review'?review():saved();bind();window.scrollTo(0,0)}
...
async function start(){if(!(S.ev&&S.ev.name&&S.ev.location)){eventBox();return;}S.chunks=[];S.interrupted=false;S.audioIncomplete=false;S.queued=false;if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia||typeof MediaRecorder==='undefined'){S.screen='review';render();return}try{S.stream=await navigator.mediaDevices.getUserMedia({audio:true});S.rec=new MediaRecorder(S.stream);S.rec.ondataavailable=e=>{if(e.data&&e.data.size)S.chunks.push(e.data)};S.rec.onstop=finish;S.rec.start(3000);acquireWakeLock();document.addEventListener('visibilitychange',onVisibilityChange);S.t0=Date.now();S.paused=false;S.pausedMs=0;S.pauseStart=0;S.screen='recording';render();S.timer=setInterval(()=>{let el=document.getElementById('time');if(el){let now=S.paused?S.pauseStart:Date.now();let s=Math.max(0,Math.floor((now-S.t0-S.pausedMs)/1000));el.textContent=Math.floor(s/60)+':'+String(s%60).padStart(2,'0')}},250)}catch(e){S.screen='review';render()}}
...
/* ============================================================
   ...
   ============================================================ */
function finish(){releaseWakeLock();document.removeEventListener('visibilitychange',onVisibilityChange);if(S.interrupted)S.audioIncomplete=true;try{if(S.chunks.length)S.blob=new Blob(S.chunks,{type:'audio/webm'})}catch(e){}if(S.stream)S.stream.getTracks().forEach(t=>t.stop());S.gutFeel=null;S.screen='gutfeel';render()}

/* ============================================================
   GUT FEEL — the user's own quick read, captured in one tap right
   after they stop. It seeds priority, is passed to the AI as a prior
   (never overriding transcript facts), and acts as a soft cost gate:
   a "just a hello" skips the expensive structuring call server-side.
   ============================================================ */
function gutOpt(v,emoji,label,sub){return '<button class="gut-opt" data-gut="'+v+'" style="display:flex;align-items:center;gap:14px;width:100%;text-align:left;background:var(--surface,#fff);border:1px solid var(--soft,#EAE2D2);border-radius:16px;padding:16px 18px;font:inherit;cursor:pointer"><span style="font-size:26px;flex:none">'+emoji+'</span><span><span style="display:block;font-size:16px;font-weight:800;color:var(--text,#181410)">'+label+'</span><span style="display:block;font-size:13px;color:var(--muted,#5C5247);margin-top:2px">'+sub+'</span></span></button>'}
function gutfeel(){return '<div class="proc-view" style="justify-content:center"><div class="proc-title" style="margin-bottom:6px">How did it go?</div><div class="proc-sub" style="margin-bottom:24px">Your quick read helps us prioritise. One tap.</div><div class="gut-opts" style="display:grid;gap:12px;width:100%;max-width:380px">'+gutOpt('strong','🔥','Real opportunity','Worth acting on soon')+gutOpt('warm','🤝','Worth keeping warm','A good connection to nurture')+gutOpt('light','👋','Just a hello','Polite, no real signal yet')+'</div><button class="gut-skip" data-gut="" style="margin-top:20px;background:none;border:0;color:var(--faint,#95887A);font:inherit;font-size:14px;cursor:pointer;text-decoration:underline">Skip</button></div>'}
function chooseGutFeel(v){S.gutFeel=v||null;enqueueOrProcess()}
...
async function enqueueOrProcess(){
  ...
  try{
    ...
    const cap={id:cid,userId:user(),name:null,company:null,role:null,
      ...
      gutFeel:S.gutFeel,completeness:0,audioIncomplete:!!S.audioIncomplete};
    const jobPayload={template:{id:t.id,name:t.name,signals:t.signals,canonical:canonicalOf(t)},
      ...
      eventContext:S.ev?{name:S.ev.name,date:S.ev.date}:null,gutFeel:S.gutFeel};
...
function saveQueuedLocal(cap){
  let c={id:cap.id,name:null,company:null,role:null,
    ...
    gutFeel:cap.gutFeel,hasAudio:true,audioDurationMs:S.dur||0,
  ...
  let list=read(K.CAP,[]);list.unshift(c);localStorage.setItem(K.CAP,JSON.stringify(list))
}
function gutPriority(g,fb){if(g==='strong')return'act-soon';if(g==='warm')return'worth-exploring';if(g==='light')return'keep-warm';return fb}
...
function save(){try{pullForm();let t=tpl(),e=S.ev,p=S.p,ai=S.ai,evText=e&&e.name?[e.name,e.session,e.location].filter(Boolean).join(' · '):'No event set';
  ...
  let completeness=ai&&ai.completeness?ai.completeness:complete(p);
  ...
  let c={id:id(),name:p.name||null,company:p.company||null,role:p.role||null,
    ...
    transcriptConfidence:S.transcriptConfidence||null,gutFeel:S.gutFeel||null,
