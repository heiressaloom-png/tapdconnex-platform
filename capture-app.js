...
/* ============================================================
   GUT FEEL — two-tap in-room read, captured right after the user stops,
   BEFORE the AI runs (the in-room read fades fast). The human reports
   what they FELT; the engine judges what it means.
     Q1 Room read (momentum):     building | steady | fading
     Q2 Their openness (recept.):  open | mixed | closed
   Sent as gutFeel: { momentum, receptivity }. Skip defaults to
   steady/mixed and never blocks. Tap only — it MUST NOT touch
   signalScore/confidence; it shapes the draft's prior, not the facts.
   ============================================================ */
var _gutSel={momentum:null,receptivity:null};
var _gutResolve=null;
function _gutTap(e){var b=e.target.closest('[data-group]');if(!b)return;var g=b.getAttribute('data-group'),v=b.getAttribute('data-value');var sib=document.querySelectorAll('[data-group="'+g+'"]');for(var i=0;i<sib.length;i++)sib[i].classList.remove('selected');b.classList.add('selected');_gutSel[g]=v;var done=document.getElementById('gut-done-btn');if(done)done.disabled=!(_gutSel.momentum&&_gutSel.receptivity)}
function _gutHide(){var scr=document.getElementById('gut-feel-screen');if(scr)scr.style.display='none';_gutResolve=null}
function showGutFeel(onComplete){_gutSel.momentum=null;_gutSel.receptivity=null;var scr=document.getElementById('gut-feel-screen');if(!scr){onComplete({momentum:'steady',receptivity:'mixed'});return}var done=document.getElementById('gut-done-btn');if(done)done.disabled=true;var btns=document.querySelectorAll('.gut-btn');for(var i=0;i<btns.length;i++)btns[i].classList.remove('selected');_gutResolve=onComplete;scr.style.display='flex'}
function _gutWire(){var scr=document.getElementById('gut-feel-screen');if(!scr)return;scr.addEventListener('click',_gutTap);var done=document.getElementById('gut-done-btn');if(done)done.onclick=function(){var r={momentum:_gutSel.momentum||'steady',receptivity:_gutSel.receptivity||'mixed'};var cb=_gutResolve;_gutHide();if(cb)cb(r)};var skip=document.getElementById('gut-skip-btn');if(skip)skip.onclick=function(){var cb=_gutResolve;_gutHide();if(cb)cb({momentum:'steady',receptivity:'mixed'})}}
_gutWire();
...
function gutPriority(g,fb){if(!g||typeof g!=='object')return fb;if(g.momentum==='building')return'act-soon';if(g.momentum==='fading')return'keep-warm';if(g.momentum==='steady')return'worth-exploring';return fb}
